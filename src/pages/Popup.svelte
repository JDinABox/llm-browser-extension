...
<div use:melt={$tabRoot} class="flex h-full w-full flex-col">
	...
	<div class="flex grow px-4 py-2 overflow-scroll">
		<div use:melt={$content('tab-1')} class="grow">
			<div class="flex items-end justify-between">
				...
				<div class="flex space-x-1.5">
					...
					<div>|</div>
					<div>
						<button on:click={clearAll} class="flex text-rose-600 font-semibold">
							Clear
						</button>
					</div>
				</div>
			</div>
			<div>{!$checked ? text : originalText}</div>
		</div>
		<div use:melt={$content('tab-2')}></div>
	</div>
	<div class="flex shrink-0 grow-0 h-9" use:melt={$list}>
